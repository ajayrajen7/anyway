...
export default function SheetShell({ children, onClose }: { children: ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-10 max-h-[80vh] overflow-y-auto rounded-t-xl bg-slate-950 p-4 shadow-lg">
      <div className="flex justify-end">
        <button type="button" onClick={onClose} aria-label="Close" className="px-2 text-slate-400">
          ✕
        </button>
      </div>
      {children}
