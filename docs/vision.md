...
3. **Outcome numbers have previously caused the user to quit.** The original design collected weight but withheld it from view for 84 days ("the Vault") rather than not collecting it at all. **UX refactor (post-M10, see `memory.md`): the owner decided against that middle ground — weight isn't tracked at all now, not logged and not revealed later.** The underlying insight stands (daily outcome numbers are demotivating noise at this signal-to-noise ratio), it's just resolved by not collecting the number rather than by collecting-and-hiding it.

## Durable principles

- **Capture, don't analyze.** v1 builds no correlation, no trend line, no insight text. The two descriptive screens (Coverage, Week Plan — split from a single Week View in the UX refactor) show counts only. Analysis is either premature (not enough data) or actively counterproductive (see constraint 3) until week 12.
