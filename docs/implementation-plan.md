...
- **M1 — Exercise library.** `seed/exercises.json` (all 75 + blocked list, §A5.4/A5.5) + a seed-loader that populates `exercises`/`exercise_muscles`. `GET /api/exercises` (search, excludes blocked unless `?include_blocked=1`). This is the highest-risk milestone per §B8 — the data *is* the product; transcribe it exactly, don't approximate.
